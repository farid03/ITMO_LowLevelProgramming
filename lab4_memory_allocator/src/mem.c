...
struct region region;
    ...
    void* mapped_address = map_pages(addr, query, MAP_FIXED_NOREPLACE);
    if (mapped_address == MAP_FAILED) {
        mapped_address = map_pages(addr, query, 0);
        if (mapped_address == MAP_FAILED) {
            return REGION_INVALID;
        }
        if (mapped_address != addr) {
            flag = false;
        }
    }
    region = (struct region) {mapped_address, query, flag};
    block_init(mapped_address, (block_size) {query}, NULL);
    return region;
}
...
static bool split_if_too_big(struct block_header *block, size_t query) {
    ...
    block_size free_block_size = (block_size) {block->capacity.bytes - query};
    block->capacity = (block_capacity) {query};
    void *sub_block = block_after(block);
    block_init(sub_block, free_block_size, block->next);
...
static struct block_header* grow_heap(struct block_header *restrict last, size_t query) {
    ...
    void* new_address = block_after(last);
    struct region new_region = alloc_region(new_address, size_max(query, BLOCK_MIN_CAPACITY));
...
static struct block_header* memalloc(size_t query, struct block_header *heap_start) {
    ...
    switch (found_block.type) {
        case BSR_REACHED_END_NOT_FOUND:
            if ((found_block.block = grow_heap(found_block.block, query)) == NULL) {
                return NULL;
            }
            found_block = try_memalloc_existing(size_max(query, BLOCK_MIN_CAPACITY), heap_start);
